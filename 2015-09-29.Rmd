...
numboot <- 500  # number of bootstrap repetitions
n <- 25  # sample size
```
...
runresamp <- function(rate=1) {
  ...
  return(data.frame(mean=mean, bottomnocover=bottomnocover, topnocover=topnocover))
}
```
...
tally(~ (topnocover & bottomnocover), format="proportion", data=res)
```
